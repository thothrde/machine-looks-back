...
const STR = {
  de: {
    ...
    chooseInfo:'Welche Information wollen Sie zuerst?', chooseAction:'Neue Evidenz liegt vor. Was tun Sie?', revealEvidence:'Evidenz anzeigen', nextScenario:'Nächste Situation', finishExperiment:'Auswertung anzeigen', endExperiment:'Experiment beenden', restart:'Neu beginnen', selected:'gewählt', scenarioOf:(a,b)=>`Situation ${a} von ${b}`, experimentLength:'3 Situationen · je 2 Entscheidungen · danach automatische Auswertung', completedExperiment:'Selbstexperiment beendet', completedCount:(a,b)=>`${a} von ${b} Situationen abgeschlossen`, summaryIntro:'Die Zusammenfassung verdichtet Ihre gewählten Informationswege, Revisionen und Entscheidungen zu einer mehrdimensionalen Denkspur.', restartExperiment:'Alle drei Situationen neu beginnen', backHome:'Zur Startseite', evidenceLabel:'Neue Evidenz', outcomeLabel:'Realisierter Ausgang', cipolla:'Cipolla-Folgenoptik', self:'Selbst', others:'Andere', clear:'klar', weak:'schwach', mixed:'gemischt', tentative:'tentativ', noSignal:'noch kein Signal',
    methodCards:[
  ...
  en: {
    ...
    experimentEyebrow:'Fictional self-experiment', experimentTitle:'Decide before you know how it turns out.', experimentSummary:'Three short situations, each with an information choice and a decision. The app then condenses your information paths, revisions and decisions into a multidimensional reasoning trace. Cipolla’s consequence lens complements the fictional part.', traceHeading:'Reasoning trace', outcomeHeading:'Outcome space',
...
function renderCases(){
  ...
  $('#case-grid').innerHTML=items.map(c=>`
    <article class="case-card">
      ...
      <h2>${txt(c.title)}</h2>
      <p>${txt(c.teaser)}</p>
      <div class="case-dims" aria-label="${t('ariaDimensions')}">${c.dimensions.map(d=>`<span class="dim-chip" title="${d}">${d}</span>`).join('')}<span class="risk-chip" title="${t('ariaRisk')}">${c.risk}</span></div>
...
function renderCaseDetail(id){
  ...
  $('#case-detail-content').innerHTML=`
    <header class="case-detail-header"><div class="case-meta"><span>${c.institution}</span><span>${c.years} · ${categoryLabel(c.category)} · ${c.risk}</span></div><h1>${txt(c.title)}</h1><p class="lead">${txt(c.teaser)}</p><div class="case-dims" aria-label="${t('ariaDimensions')}">${c.dimensions.map(d=>`<span class="dim-chip">${d}</span>`).join('')}</div></header>
...
function renderTrace(){
  $('#trace-list').innerHTML=DIMENSIONS.map(d=>{
    const sig=experiment.traces[d.id]; const label=sig ? t(sig) : t('noSignal');
